	...
	//if (!isset($_GET["Id"]) || trim($_GET["Id"]) == "" ) {		// Attempt new record
		...
		if (!$PreExisting) { // no pre-existing -> Insert new record
			...
			if (!isset($_GET["action"]) || trim($_GET["action"]) <> "edit") {
				...
				if (!isset($PermitNumber) || (isset($PermitNumber) && 
				trim($PermitNumber) == "")) {
					
					if ($db) {echo '<br>No permit number -> will assign';}
					$PermitNumber = "Emergency-" . $Id; // favoring submission id over first detail id
					$PermitNumber = "Emergency-" . $SubmissionId;
					if ($db) {echo "<br>Made-up permit number |" .  $PermitNumber . "|<br>";}
					
					$query = "Update DNDetails set permit_number = '" . 
					$PermitNumber . "' Where Id = " . $Id;
					
					if ($db) {echo '<br>Emergency permit number assigned: ' . $query;}
					$result = mysqli_query($connection, $query) or 
					die("<br>Query failed" . " " . $query . " " . mysqli_error($connection));			
				}	
