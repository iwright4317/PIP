...
If (isset($_GET['action'])) {
	if (trim($_GET['action']) == "cycles" || trim($_GET['action']) == "monthreset") {
		$q = "Delete from Cal where Action = '$_GET[action]'";
		$r = mysqli_query($connection,$q) or die("<br/>Query failed - 18" . $q);
	}
	
	if (trim($_GET['action']) == "remove") {
		$q = "Delete From Cal where Action = '$_GET[item]' and aValue = '$_GET[value]'";
		$r = mysqli_query($connection,$q) or die("<br/>Query failed - 18" . $q);
	}
	...
	if (trim($_GET['action']) == "reset") {
		...
		$q = "Insert into Cal (Action,aValue) values ('$_GET[action]','$_GET[value]')";
		$r = mysqli_query($connection,$q) or die("<br/>Query failed - 18" . $q);
	}
}
	...
	<div class="container" style='font-size medium; font-weight: normal;'> <?php
		...
		<div class="row">
			<div class="col-sm-12" style='text-align:center; font-size: medium; font-weight: normal;'>
				<a href=cal2.php?action=reset>Reset</a>&nbsp;&nbsp;
				Read cycles: <?php
				if ($intCycles == 18) { ?>
					<a href=cal2.php?action=cycles&value=18><b>18</b></a>&nbsp; <?php
				} else { ?>
					<a href=cal2.php?action=cycles&value=18>18</a>&nbsp; <?php	
				}		

				if ($intCycles == 19) { ?>
					<a href=cal2.php?action=cycles&value=19><b>19</b></a>&nbsp; <?php
				} else { ?>
					<a href=cal2.php?action=cycles&value=19>19</a>&nbsp; <?php	
				}		

				if ($intCycles == 20) { ?>
					<a href=cal2.php?action=cycles&value=20><b>20</b></a>&nbsp; <?php
				} else { ?>
					<a href=cal2.php?action=cycles&value=20>20</a>&nbsp; <?php	
				}		

				if ($intCycles == 21) { ?>
					<a href=cal2.php?action=cycles&value=21><b>21</b></a>&nbsp; <?php
				} else { ?>
					<a href=cal2.php?action=cycles&value=21>21</a>&nbsp; <?php	
				}		

				if ($monthreset) { ?>
					<a href=cal2.php?action=monthreset&value=false><b>Monthly reset</b></a>&nbsp;&nbsp; <?php
				} else { ?>
					<a href=cal2.php?action=monthreset&value=true>Monthly reset</a>&nbsp;&nbsp; <?php
				} ?>				
				<a href=cal2.php?printer=yes>Word version</a>&nbsp;&nbsp;
...
while ($intMonth <= 12) {
    ...
    $strTitle = "Read Calendar for " . $strMonthName . " " . $intYear;
	echo "$strTitle";
    ...
    For ($X = 1; $X <= $intNumberOfDays; $X++) {
        ...
        date('w', strtotime($strCurrentDay)) < 6) || $pullDay) {
			if(trim($holiday) == "" || $pullDay) {
        		...
        		if (!$printer && $OK) {
        		echo "<a href=cal2.php?action=push&value=$strCurrentDay>push</a>&nbsp;
        		<a href=cal2.php?action=pull&value=$strCurrentDay>pull</a>";
				}
        		echo "<br>$holiday";
				...
				while ($pullCount >= 1) { 
    				...
    				if ($OK) {echo "<br>Read Cycle " . $CurrentCycle . "<br>";
    				echo $rDays . " day billing cycle"; }
				...
				if ($pullDay && $OK) {echo "<br><a href=cal2.php?action=remove&item=pull&value=$strCurrentDay>remove pull</a>";}
				
			} else {
    			echo "<td style='padding-left: 5px; padding-right: 5px; $stBackGround' id='$X' align='left' valign='top' >";
        		echo "<a name='$X'><b>$X</b></a>&nbsp;";        		
        		if (!$printer && $OK) {
        			echo "<a href=cal2.php?action=pull&value=$strCurrentDay>add</a>";
				}
        		echo "<br>$holiday";		
			}
        	...
        	echo "<a name='$X'><b>$X</b></a>&nbsp;";    		
				...
				<br>
				<a href=cal2.php?action=remove&item=push&value=$strCurrentDay>remove push</a>";}
				if ($pullDay && $OK) {echo "<br><a href=cal2.php?action=remove&item=pull&value=$strCurrentDay>remove pull</a>";}				
		}
...
while ($icycle <= $intCycles) {
    ...
    while ($imonth <= 14) {
        $bDays = '';
        $ebDays = '';
        if($ReadDays[$icycle][$imonth] >= $bValue) {
			$bDays = "<b>";
			$ebDays = "</b>";
        }
