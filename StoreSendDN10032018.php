...
$DNEmail = $DNEmail . '</td>';
$DNEmail = $DNEmail . '<td>';
$DNEmail = $DNEmail . '<i>Civil Inspections</i> at <u>piutildocuments@vbgov.com</u>, <u>rawillia@vbgov.com</u>,  ';
...
while ($row = mysqli_fetch_array($result)) {				
	...
	(`PermitStatus` = 'APPROVED' or `PermitStatus` = 'APPROVED WITH CONDITIONS' or `PermitStatus` = 'BLANKET' or `PermitStatus` = 'EMERGENCY'  or `PermitStatus` = 'RIGHT-OF-WAY') order by Id desc";
...
if (trim($PermitType) != "") {	
	$DNEmail = $DNEmail . '	<br>' .  $PermitType . '</b>';
}
$DNEmail = $DNEmail . '</td>';
$DNEmail = $DNEmail . '<td style="border: 1px solid black; ">&nbsp;';
