...
<head>
	...
	<script>
		function deleteLine(Id,location) {
			var txt;
			//var location;
			...
			if (r == true) {
 			   txt = "You pressed OK!";
 			   window.location = "http://pip.vabeachpu.com/UtilityVerificationApplication.php?chkDelete=on&UVACUsId=" + Id;
			} else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	...
	(isset($_GET["chkDelete"]) && trim($_GET["chkDelete"]) == "on")) {
		if ($UVACUsId <> 0) {
		...
		(!isset($_POST["chkDeleteUVA"]) || trim($_POST["chkDeleteUVA"]) <> "on")) {
			...
			StreetName, Company, tComments) values (" . $PIPId . ",	
			'" . $_POST['Verified'] . "', 
			'" . str_replace("'","''",$_POST['Technician']) . "', 
			'" . str_replace("'","''",$_POST['txtPermitAddress']) . "', 
			...
			LocationAddress = '" . str_replace("'","''",$_POST['txtPermitAddress']) . "' and 
		...
		if ($UVAId <> 0 ) {
			...
			LocationAddress = '" . str_replace("'","''",$_POST['txtPermitAddress']) . "', 
			...
			if ($NewUtility || (isset($_GET["action"]) && trim($_GET["action"]) == "NewUtility")) {
...
include ('./includes/database2.inc');
		...
		<div class="row">
			<div class="col-sm-12" style="border-top: 1px grey solid">
				&nbsp;Technician&nbsp;comments:<br>
				<input type="textbox" name="tComments" style="width:100%"
				<?php
					if ($edit) {if (isset($tComments)) {echo "value='" . $tComments . "' ";}
					}
 				?>>
			</div>		
		</div>		
		...
		<div class="row">
			<div class="col-sm-12" style="border-top: 2px solid black; font-size: large; font-weight:bold; background-color: whitesmoke; ">
				Utility crossing location
			</div>
		</div>		
		
		<div class="row" style="border-top: 1px solid grey; background-color: whitesmoke">
		...
		<div class="row">
			<div class="col-sm-6" style="font-size: x-large; ">
		...
		<div class="row" style="font-weight:bold; ">
			...
			<div class="col-sm-2" style="text-align: center; border-bottom: 1px solid black; vertical-align: bottom">
				Utility Crossings
			</div>
			<!-- 
		...
		//if ($UVAId <> 0) {
			...
			if ($PIPId <> 0) {
				...
				While ($row = mysqli_fetch_array($r)) {				
					?>		
					<div class="row" style="font-weight:bold; ">
						<div class="col-sm-1" style="height:24px; border-bottom: 1px solid black; border-right: 1px solid black; ">
							<a href="UtilityVerificationApplication.php?UVAId=<?php echo $row[1];?>"><?php echo $UVACount;?></a>
						</div>
						<div class="col-sm-2" style="height:24px; text-align:center; border-bottom: 1px solid black; border-right: 1px solid black; ">
							<?php echo '&nbsp;' . $row[2] . '&nbsp;';?>
						</div>
						<div class="col-sm-6" style="height:24px;border-right: 1px solid black; border-bottom: 1px solid black; vertical-align: bottom">
							<a href="UtilityVerificationApplication.php?UVAId=<?php echo $row[1];?>">
							<?php echo str_replace(", Virginia Beach, VA, United States", "", $row[0]) . ' / ' . $row[3];?>
							</a>
						</div>
						<div class="col-sm-2" style="height:24px; text-align: center; border-bottom: 1px solid black; vertical-align: bottom">
							<?php 
							$q2 = "Select count(*) from UVACUs  
							Where UVAId = " . $row[1] . " and EUType is not Null";
						...
						<!-- 
						<div class="col-sm-1" style="text-align: center; border-bottom: 1px solid black; vertical-align: bottom">
							<a title="Delete this crossing" 
							onclick="deleteLine('. $row[1] . ',' . $row[0]) . ' / ' . $row[3] . ');">
							<img src="delete.jpg" style="height:23px"></a>
						</div>
						-->
					</div>		
					<?php
