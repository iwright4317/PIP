...
<head>
		...
		function validateandsubmit(theform) {
			...
			if (CheckForInput) {
				...
				if (theform.permit_number != null && theform.epermit != null) {
					if (theform.permit_number.value == 'New permit' && isblankentry(theform.epermit)) {					
						$txtStatus = $txtStatus + "<br>You have selected New permit, but no permit number has been entered into the enter permit # text box. Please enter a permit into the enter permit # text box and click the Save button again. ";
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#permit_number").change(function() {
				//alert(document.getElementById("permit_number").value);
				if (document.getElementById("permit_number").value == "Emergency" || document.getElementById("permit_number").value == "New permit") {
					//alert('emergency');
...
if (isset($_GET["Id"]) && trim($_GET['Id']) <> "" && !is_numeric($_GET["Id"]) ) {exit;} // might be a hack
	...
	<div class="row">
		<div class="col-sm-6" >
			...
			if ($OrganizationType == 'Utility Company') {
				echo 'Utility  Company: <br><b>' . $Organization . '</b>
				<input type="hidden" id="utilityCompany" name="utilityCompany" 
				value="' . $Organization . '">';
			} else {
				...
				if ($edit and !$found) {
					echo '<option selected>' . $utilityCompany . '</option>';
				}
				echo '</select>';
				} else {
					if (isset($utilityCompany)) {echo '<b>' . $utilityCompany . '</b>';}
				}
			}			
			?>
		</div>
	...
	<div class="row">
		<div class="col-sm-4" style="">
			<?php			
				...
				If ($edit) {
					$q = "Select PermitNumber, Id from PIP Where CompanyName = '" . $utilityCompany . "' and 
					...
					$q = "Select PermitNumber, Id from PIP Where CompanyName = '" . $Organization . "' and 
					(DateOfSignature between '" . $myDate . "' and '" . $today . "')  Order by PermitNumber" ;
				}			
			...
			If (isset($_GET["action"]) && trim($_GET["action"]) == "nl") {
				...
				if ($OrganizationType == 'Utility Company' || $edit) {	
					...
					while ($row = mysqli_fetch_array($r)) {
						if ($edit) {							
							...
							(!isset($_GET["action"]) || (isset($_GET["action"]) && $_GET["action"] <> "np"))) {
								echo "<option  selected>" . $row['PermitNumber'] . "</option>";
								$found = TRUE;
							} else {
								echo "<option  >" . $row['PermitNumber'] . "</option>";
							}
						} else {
							echo "<option  >" . $row['PermitNumber'] . "</option>";
						}					
					...
					if (!$found || True) {
						if (isset($permit_number)) {
						echo "<option selected >" . $permit_number . "</option>";
						}
					}
				} else {
		...
		<div class="col-sm-2" style="">
			...
			?>
			Arrival&nbsp;Time:<input type="text" id="ph" name="ph" style="visibility: hidden; width:1px"><br>			
	...
	<div class="row" >
		<div class="col-sm-12" style="">
			Permit&nbsp;Address:<br>
			<div name="StreetAddress" id="StreetAddress" ><b><?php  if(isset($StreetAddress)) {echo $StreetAddress;}?></b></div>	
