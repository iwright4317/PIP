...
function findString (str) {
 ...
 return;
	...
	<script>
		$(document).ready(function () {
        $('#select-anchorT').change( function () {
            var targetPosition = $($(this).val()).offset().top;
            $('html,body').animate({ scrollTop: targetPosition}, 'slow');
        });
    });
...
include ('./includes/database2.inc');
	...
	<div class="container"  style="font-size : normal; font-weight: normal; background-color: white">
		...
		<div class="row">
			<div class="col-md-6" style="font-size: x-large; font-weight: bold;">
				Daily Notifications	for <?php echo date_format(date_create($DNDate),"m/d/Y"); ?>		
			</div>
			<div class="col-md-6" style="font-size: medium; text-align: right">
            ...
            $strSQL = "Select PIP.PermitNumber, DNDate, PIP.Technician " . 
...
if (isset($_GET["Id"]) && trim($_GET['Id']) <> "" && !is_numeric($_GET["Id"]) ) {exit;} // might be a hack
            ...
            If ($txtAnd == " and ") {
				...
				if ($db) {echo '<br>strSQL: ' . $strSQL . ' :';}
				
	if (!$printer) {
		$ddlDNDate = "";
		
		if (isset($DNDate)) { ?>
			<a title="download your permits" href=PreviewDN2.php?DNDate=<?=$DNDate?>&printer=yes><b>Print mine</b></a>&nbsp;&nbsp;&nbsp;
	...
	if (!$printer) {
				...
				echo "<select name='select-anchorT'  id='select-anchorT'>";		
				...
				While ($i <= $MaxI) {
					if ($CurrentTechnician <> $Technicians[$i]) {
						echo '<option value="#' . $Technicians[$i] . '">' . $Technicians[$i] . '</option>';                                      
					}
		...
		<div class="row" >
			<div class="col-md-12" style="text-align: right">
                ...
                //$strBody = $strBody . "<tr bgcolor=#FFFFFF><th style='padding-left: 5px; padding-right: 5px;' colspan=5><h3>&nbsp;
                //Daily Notifications for " . date_format(date_create($DNDate),"m/d/Y") . "</h2></th></tr>";
...
<tr bgcolor=" . $BGShade . ">";
                        $strBody = $strBody . "
<td style='padding-left: 5px; padding-right: 5px;' align=center>" . $PermitNumbers[$i] . "</td>
...
<td style='border-bottom: 2px black solid;  padding-left: 5px; padding-right: 5px;' colspan=5>";                        
                        ...
                        $strBody = $strBody . "" . nl2br($Condition) . "</td></tr>";					
...
$strBody = $strBody . "</tr>";
		...
		if (trim($Contractor) == "") {
			...
			//$strBody = $strBody . "<tr bgcolor=" . $BGShade . "><td colspan='5'>&nbsp;</td></tr>";
			//$strBody = $strBody . "<tr bgcolor=" . $BGShade . ">
			//<th style='padding-left: 5px; padding-right: 5px;'  colspan='5'>&nbsp;</th></tr>";
		}
						...
						$strBody = $strBody . '<br style="page-break-before: always">';
                    } 
                    If ($First) { // No open permit record for this permit number
                    
						$strBody = $strBody . "<TABLE width=100% border=1 >";
                        $strBody = $strBody . "<tr bgcolor=" . $BGShade . "><td style='padding-left: 5px; padding-right: 5px;' colspan=7><b>Error: No open permit record for permit number: " . 
